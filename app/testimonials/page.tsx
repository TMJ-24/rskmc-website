...
const staticTestimonials: Testimonial[] = [
  { id: "1", name: "Sister Grace Naime",    role: "Church Member — 8 years", quote: "RSKMC has been my spiritual home for eight years. The teaching is rooted in the Word of God and the community genuinely cares for one another. I came broken, and this church helped me find wholeness in Christ.", highlight: true,  order: 1, published: true, createdAt: "", updatedAt: "" },
  { id: "2", name: "Brother Joseph Katu",   role: "Youth Leader",             quote: "Joining the youth ministry here transformed my walk with God. The leadership invested in me personally, and I saw my faith grow in ways I never expected. This church is truly a family.",                                 highlight: false, order: 2, published: true, createdAt: "", updatedAt: "" },
  { id: "3", name: "Evelyn & Peter Rosi",   role: "Married Couple",           quote: "We were married here and have watched our family grow in faith together. The pastoral care we received during a difficult season was a true reflection of Christ's love. We are forever grateful.",                          highlight: false, order: 3, published: true, createdAt: "", updatedAt: "" },
  { id: "4", name: "Deaconess Mary Tora",   role: "Women's Ministry Leader",  quote: "The legacy of Rev Sione Kami is alive in this congregation. His heart for the community, the poor, and the lost continues to shape everything we do. It is a privilege to serve here.",                                   highlight: false, order: 4, published: true, createdAt: "", updatedAt: "" },
  { id: "5", name: "Pastor David Lewa",     role: "Worship Ministry",         quote: "I have been in many churches but the depth of worship at RSKMC is different. There is a genuine hunger for God's presence here, and it pushes me closer to Him every single week.",                                        highlight: false, order: 5, published: true, createdAt: "", updatedAt: "" },
  { id: "6", name: "Sister Rachel Ume",     role: "New Member — 1 year",      quote: "I was a new believer when I walked through these doors. The patience, love, and discipleship I received helped me build a strong foundation in faith. I am so glad I found RSKMC.",                                         highlight: false, order: 6, published: true, createdAt: "", updatedAt: "" },
  { id: "7", name: "Brother Samuel Vatu",   role: "Board Elder",              quote: "The vision of this church has never wavered. From the very beginning, it has been about honouring God and serving people. That consistency is rare, and I believe it is the fruit of God's hand upon us.",                  highlight: false, order: 7, published: true, createdAt: "", updatedAt: "" },
  { id: "8", name: "Mama Ruth Kapi",        role: "Senior Member — 12 years", quote: "This church prayed with me through sickness, through loss, and through breakthrough. They have walked every chapter of my life with me. That kind of love does not come from people alone — it comes from God.",          highlight: false, order: 8, published: true, createdAt: "", updatedAt: "" },
] as Testimonial[];
...
export default function Testimonials() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const published = res.data.filter((t) => t.published !== false);
        const sorted = [...published].sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
        setItems(sorted.length > 0 ? sorted : staticTestimonials);
      } catch {
  ...
  const featured = items.find((t) => t.highlight);
  ...
  return (
    <div>
      ...
      {loading ? (
        <div className="py-24 text-center text-gray-400">Loading testimonials…</div>
      ) : (
        <>
          {/* Featured */}
          {featured && (
